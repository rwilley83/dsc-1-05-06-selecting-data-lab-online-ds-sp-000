...
def select_all_planets_with_mass_greater_than_one():
    return cursor.execute('''SELECT * FROM planets WHERE mass > 1;''').fetchall()

def select_name_and_mass_of_planets_with_mass_less_than_equal_to_one():
    return cursor.execute('''SELECT name, mass FROM planets WHERE mass >= 1;''').fetchall()
...
def select_all_planets_with_moons_and_mass_less_than_one():
    return cursor.execute('''SELECT name, color FROM planets WHERE num_of_moons >= 1 AND mass < 1;''').fetchall()

def select_name_and_color_of_all_blue_planets():
    return cursor.execute('''SELECT name, color FROM planets WHERE color LIKE '%blue%';''').fetchall()
